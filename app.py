...
def cache_services():
    ...
    return all_movie_services, all_tv_services, all_movie_services_json, all_tv_services_json, sorted(list(set(movie_services))), sorted(list(set(tv_services)))
...
def get_servicies():
    ...
    st.selectbox("Select country", countries_sorted, key="country", accept_new_options=False, placeholder="Type country", index=get_countries_index(), help=tooltip)
...
def get_countries():
    ...
    st.multiselect("Select service", select_services, key="services", accept_new_options=True, placeholder="Type service", default=None, max_selections=4, help=tooltip)
    ...
    if services:
        ...
        for iii, service in enumerate(services):
            ...
            with cols[iii]:
                display_image_with_hover(f"https://image.tmdb.org/t/p/original/{service_logo_path}.png?api_key={_tmdb.API_KEY}", service)
                for type in types:
                    st.subheader(f"{type}: ")
